...
`Vesper Service and Lord's Table.docx` is the canonical content source for the 12 Lord's Discourses and their Closing texts, the four Words of Institution readings, and the four Great Commission Text readings. `lib/liturgy/vesperTableRotation.ts` holds the transcribed options and computes all four results. New Vesper liturgies currently place only the Discourse, Words of Institution, and Closing selections; Great Commission placement remains deliberately open because that Section is modeled as a Formula, not a Selection — a real scope decision, not scheduled.
