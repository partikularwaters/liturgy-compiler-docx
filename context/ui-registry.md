...
- `components/liturgy/LiturgyDateRow.tsx` — shared liturgy date/status row. Used by both `/liturgies` (since 2026-07-29) and the Homepage's "Recent Liturgies" preview (since 2026-08-27) — the two must stay on this one shared component/grouping helper rather than drift into separate implementations again. `readOnly?: boolean` (2026-08-28, default `false`) suppresses the whole per-liturgy options menu regardless of `currentUser` — the Homepage preview passes it since that list is purely display, not management; `/liturgies` keeps the default. **Established pattern:** any future read-only rendering of a normally-interactive shared row component should add a `readOnly` prop like this one, not a duplicated component or a `currentUser={null}` workaround at the call site. **Redesigned 2026-08-30**: a left-anchored Lord's Day #/Special Service container, then a Morning container and a Vesper container (each `border border-border rounded-md px-3 py-2`, background left uncolored by explicit request), each tagged with a left-anchored colored pill — "Morning" (`bg-info-light`/`text-info-foreground`) and "Vesper" (`bg-warning-light`/`text-warning-foreground`). These tags land exactly on `ui-rules.md`'s existing Badges spec (2px/10px padding, 11px/500 weight, pill radius) without having been deliberately copied from it — now added there as two new established variants. Each liturgy's own trailing trash icon was replaced by `components/liturgy/LiturgyOptionsMenu.tsx` (a per-liturgy 3-dot menu: Edit / Mark as Ready — only rendered at all when actually complete, never just disabled / Web Link / Delete — same dropdown-entrance and press-feedback recipe as `AccountMenu.tsx`/`TopNavLinks.tsx`), and Delete now opens `components/liturgy/ConfirmDeleteLiturgyDialog.tsx` rather than a native `window.confirm`.

### ConfirmDeleteLiturgyDialog
File: `components/liturgy/ConfirmDeleteLiturgyDialog.tsx`

Built on the existing `Modal.tsx` (reused for its full a11y contract — dialog semantics, focus trap, Escape — never reinvented). A GitHub-style type-your-own-name-to-confirm gate: the dialog prints the signed-in Compiler's own account name and requires an exact match before the destructive button enables; a Curator has no typed gate at all (see `deleteLiturgyAction.ts`). Pair-aware selection when the triggering liturgy has a paired sibling on the same date: two toggle buttons (`border-border`/`text-text-muted`/`bg-surface-secondary` unselected → `border-error`/`text-error`/`bg-error-light` selected), the triggering liturgy always pre-selected and non-toggleable. **Established pattern for any future "delete N related things one call at a time" flow:** tracks a `deletedIds` set across the sequential per-item calls, refreshing the underlying list after *each* individual success (not batched until the whole loop finishes) and naming exactly which item failed if the loop stops partway — never leave a multi-item destructive flow's UI implying "nothing happened" when part of it already did.
