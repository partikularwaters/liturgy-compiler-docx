...
`components/reader/ReaderTargetPicker.tsx` selects the destination Liturgy/Section without moving Section-scope authority into the client. A Compile arrival shows the resolved Liturgy and Section as a locked target, so a new Selection cannot accidentally be placed elsewhere. A plain Reader or Library arrival leaves both target modes active. Its mode-toggle buttons and "Set Target" button (2026-08-28, Phase 3) carry the same hover/press recipe as the rest of the app's buttons — shown once per Reader session, not a frequent control.
