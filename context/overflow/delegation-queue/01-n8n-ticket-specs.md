# Delegation Queue Archive — n8n Liturgy Automation Full Ticket Specs (Tickets 1-7)

Archived from `context/delegation-queue.md`'s "Full ticket specs" section on
2026-09-02 — all seven tickets are fully built, `survey`-verified, committed,
and live in Production (see `context/progress-tracker.md`'s Completed section
for outcomes and verification evidence). The live queue keeps only the
compact status checklist; this file is the original per-ticket spec text
(pattern to match, files to touch/not touch, done-when criteria), read on
demand if a future session needs to know exactly what was asked for, not
loaded by default.
