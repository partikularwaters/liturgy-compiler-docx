...
**2026-07-23 redesign of this same `kind` field, direct feedback:** Madrid: "I don't think the Guide is relevant any more since we have guides already... The Guide, if we're talking about the outline, can be triggered in the Compile view already." The original `kind` conflated two unrelated facts (audience, and real-content-vs-checklist); split into `kind: "corporate" | "leader"` (audience only, now driving a real derived `leaderOnly` in `resolveItemText.ts`'s "prayer" case the same way Formula/Verbal Cue's `visibility` already does — previously every Prayer showed in both Bulletin and Guide unconditionally, a real gap this closes) and a new independent `is_guide: boolean` column (placeability, replacing the old `kind = 'guide'` value). `PrayerForm`'s Kind select is now Corporate/Leader only; the old Guide option became a small, de-emphasized checkbox instead of competing with the two real audience choices. Migration `20260723020000_prayer_kind_redesign.sql` — **run by Madrid (2026-07-24), fully closed out.** Adds `is_guide`, backfills it from the old `kind = 'guide'` rows, reclassifies the real `kind` values (Confession of Sin → `corporate`; everything else, including the 6 guide rows whose `kind` is now inert → `leader` — matched by `section_name`/`is_guide`, not hardcoded IDs, per Madrid's confirmation), then swaps the check constraint and default to match. Verified live end-to-end (a real placed `leader`-kind Prayer correctly appears in the Guide docx export and not the Bulletin) — see Session Memory Bank.
...
1. **Item storage migration — ✅ shipped 2026-07-28 (migration + app code; live cutover pending Madrid running the migration + backfill script — see `progress-tracker.md`).** `sections.items` jsonb array → a proper child table (`section_items`), one row per item. Moved here from the original v2 draft: its two real justifications (pairing with Section editing, and enabling search/duplicate-flagging/tagging) both now live in v3, and nothing in v2's scope above needs per-item queryability — docx generation reads a whole liturgy in one shot exactly like PDF generation does today, jsonb is fine for that access pattern.
