...
1. **Docx export**, built in the cloned repo. Replaces `@react-pdf/renderer` as the export mechanism going forward. **PDF export is frozen in this repo — untouched, fully working, no further investment — and only actually deleted in the clone**, once docx is proven stable there.
2. **Continuous-flow authoring + manual column-break overrides**, built in the cloned repo, depends on #1. Resolves the old fixed-vs-continuous-layout question in favor of continuous flow: Word's native multi-column layout already *is* continuous flow (fills column 1, spills to column 2 automatically), and a "push to next column" override maps directly onto a real Word column-break — no custom pagination engine needed, unlike what this would have cost against the PDF renderer. This also **resolves Vesper's 3-column layout** as a side effect — there's no more per-Section page/column number to assign for either template, just Section order plus optional manual breaks — so Vesper's layout is no longer a separate build item, just confirming Vesper's actual Section grouping/order once continuous flow exists.
...
**v2 is fully closed out in this repo as of 2026-07-21** (items 3-5 done, item 6 downgraded to backlog). Only items 1-2 (docx export, continuous-flow authoring) remain — both were always scoped for a separate cloned repo, never this one, and haven't been started.
